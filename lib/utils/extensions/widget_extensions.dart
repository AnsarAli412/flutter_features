...
import '../builders/page_route_builder.dart';
...
extension WidgetExtension on Widget {
  ...
  Container border ({Color color = borderColor,double width = 1.0,double borderRadius = 6.0, double borderPadding = 0.0 }){
    return Container(
      padding: EdgeInsets.all(borderPadding),
      decoration: BoxDecoration(
        border: Border.all(color:color,width: width),
        borderRadius: circularBorderRadius(borderRadius:borderRadius )
      ),
      child: this,
    );
  }

  ShimmerBuilder shimmer({Color? highlightColor,Color? baseColor}){
    return ShimmerBuilder.fromColors(baseColor: baseColor??Colors.grey.withOpacity(.5),
  ...
  /// Launch a new screen
  Future<T?> launchWidget<T>(BuildContext context,
      {bool isNewTask = false,
        PageRouteAnimation pageRouteAnimation = PageRouteAnimation.Fade,
        Duration duration = const Duration(milliseconds: 500)}) async {
    if (isNewTask) {
      return await Navigator.of(context).pushAndRemoveUntil(
        buildPageRoute(
            this, pageRouteAnimation, animationDuration: duration),
            (route) => false,
      );
    } else {
